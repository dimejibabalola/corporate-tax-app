...
import { Upload, FileText, Check, BookOpen, Scale, ScrollText, Save, Edit2, ChevronDown, Eye, ChevronRight } from "lucide-react";
import { useState, useEffect } from "react";
import { db, Textbook as ITextbook, Chapter as IChapter, Section as ISection } from "@/lib/db";
import { detectFullStructure, generateChunksFromText } from "@/lib/parser";
...
const TextbookPage = () => {
    ...
    // Auto-initialize if no book is found
    useEffect(() => {
        if (!masterBook && !isProcessing) {
            initializeCourse();
        }
    }, [masterBook, isProcessing]);
    ...
    useEffect(() => {
        if (expandedChapter && masterBook) {
            ...
            // Load both chunks and sections for the chapter
            Promise.all([
                db.chunks.where('chapterId').equals(expandedChapter).toArray(),
                db.sections.where('chapterId').equals(expandedChapter).sortBy('sequenceOrder')
            ]).then(([chunks, sections]) => {
    ...
    const initializeCourse = async () => {
        ...
        try {
            ...
            const { chapters: parsedChapters, sectionsByChapter } = detectFullStructure(text);
            ...
            await db.transaction('rw', db.textbooks, db.chapters, db.sections, db.chunks, async () => {
                ...
                const newChapters = parsedChapters.map(c => ({
                    id: uuidv4(),
                    textbookId,
                    number: c.number,
                    title: c.title,
                ...
                // Store sections for each chapter
                const allSections: ISection[] = [];
                newChapters.forEach((nc, idx) => {
                    const chapterNum = parsedChapters[idx].number;
                    const sections = sectionsByChapter.get(chapterNum) || [];
                ...
                await db.sections.bulkAdd(allSections);

                // Create chapter map for chunker
            ...
            toast.success("Fundamentals of Corporate Tax loaded successfully!");
    ...
    if (!masterBook && !isProcessing) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] max-w-md mx-auto text-center space-y-6">
                <h2 className="text-2xl font-bold">Loading Corporate Tax Course...</h2>
                                ...
                                <CollapsibleContent className="bg-muted/10 border-t p-4">
                                    {chapterDetails && expandedChapter === chapter.id ? (
                                        <div className="space-y-6">
                                            {/* Module Sections - Primary Display */}
                                            <div>
                                                <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3 flex items-center gap-2">
                                                    <BookOpen size={14} /> Module Structure ({chapterDetails.sections.length} sections)
                                                </h4>
                                                {chapterDetails.sections.length > 0 ? (
                                                    <div className="border rounded-md bg-white dark:bg-neutral-900 divide-y">
                                                        {chapterDetails.sections.map((section, idx) => (
                                                            <div
                                                                key={idx}
                                                                className={`p-2 flex items-center gap-3 text-sm ${section.type === 'section' ? 'font-semibold' :
                                                                        section.type === 'subsection' ? 'pl-6 text-muted-foreground' :
                                                                            'pl-10 text-xs text-muted-foreground/80'
                                                                    }`}
                                                            >
                                                                <span className={`
                                                                    px-2 py-0.5 rounded text-[10px] font-mono
                                                                    ${section.type === 'section' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300' :
                                                                        section.type === 'subsection' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/50 dark:text-purple-300' :
                                                    ...
                                                    <p className="text-xs text-muted-foreground italic">No sections detected in this chapter</p>
                                                )}
                                            </div>

                                            {/* Legal References - Secondary */}
                                            <div className="grid grid-cols-2 gap-6 pt-4 border-t">
                                                <div>
                                                    <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-2">
                                                        <Scale size={14} /> Case Law ({chapterDetails.cases.length})
                                                    </h4>
                                                    {chapterDetails.cases.length > 0 ? (
                                                        <ScrollArea className="h-[120px] border rounded-md p-2 bg-white dark:bg-neutral-900">
                                                        ...
                                                        <p className="text-xs text-muted-foreground italic">No cases cited</p>
                                                    )}
                                                </div>
                                                <div>
                                                    <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-2">
                                                        <ScrollText size={14} /> Statutes ({chapterDetails.statutes.length})
                                                    </h4>
                                                    {chapterDetails.statutes.length > 0 ? (
                                                        ...
                                                        <p className="text-xs text-muted-foreground italic">No statutory references</p>
                                                    )}
                                                </div>
                                            </div>
                                        </div>
